    ...
    return (
      <div className="flex flex-1 items-center justify-center min-h-[60vh]">
        <div className="text-center animate-fade-in">
          <div className="mb-6 h-10 w-10 animate-spin rounded-full border-2 border-[#0071e3] border-t-transparent mx-auto" />
          <p className="text-xl text-[#1d1d1f] font-medium">Researching {ticker}</p>
          <p className="mt-2 text-[#6e6e73]">Querying multiple AI agents…</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-1 items-center justify-center min-h-[60vh]">
        <div className="text-center animate-fade-in">
          ...
          <p className="text-xl text-[#1d1d1f] mb-4">{error}</p>
    ...
    <div className="animate-fade-in">
      ...
      <section className="bg-[#f5f5f7] py-16 px-6">
        <div className="mx-auto max-w-4xl">
          <div className="flex items-center justify-between mb-8">
            <Link href="/" className="text-[#0071e3] hover:underline text-sm font-medium">
              ← New search
            </Link>
            {user && (
              <button
          ...
          <div className="text-center animate-fade-in">
            <p className="text-sm text-[#6e6e73] mb-2">{research.sector}</p>
            <h1 className="text-5xl md:text-6xl font-semibold text-[#1d1d1f] mb-3">
              {research.company_name}
            </h1>
            <p className="text-lg text-[#6e6e73] font-mono">{research.ticker}</p>
          </div>
            ...
            <div className="mt-10 text-center animate-fade-in-delay">
              <div className="inline-flex items-baseline gap-4">
                ...
                {quote.change_pct !== null && (
                  <span className={`text-2xl font-medium ${quote.change_pct >= 0 ? "text-[#34c759]" : "text-[#ff3b30]"}`}>
                    {quote.change_pct >= 0 ? "+" : ""}{quote.change_pct.toFixed(2)}%
                  </span>
                )}
              </div>
              <div className="mt-4 flex justify-center gap-8 text-sm text-[#6e6e73]">
                {quote.market_cap && (
                  <div>
                    <span className="block text-xs text-[#86868b] uppercase tracking-wider">Market Cap</span>
                    <span className="font-medium">{quote.market_cap}</span>
                  </div>
                )}
                {quote.fifty_two_week_high && (
                  <div>
                    <span className="block text-xs text-[#86868b] uppercase tracking-wider">52w High</span>
                    <span className="font-medium">${quote.fifty_two_week_high.toFixed(2)}</span>
                  </div>
                )}
                {quote.fifty_two_week_low && (
                  <div>
                    <span className="block text-xs text-[#86868b] uppercase tracking-wider">52w Low</span>
                    <span className="font-medium">${quote.fifty_two_week_low.toFixed(2)}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      <section className="py-16 px-6">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="rounded-3xl bg-[#fbfbfd] border border-[#d2d2d7] p-8 md:p-12 mb-12">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div>
                <h3 className="text-lg font-semibold text-[#34c759] mb-4 flex items-center gap-2">
                  <span className="w-8 h-8 rounded-full bg-[#34c759]/10 flex items-center justify-center text-sm">↑</span>
                  Bull Case
                </h3>
              ...
              <div>
                <h3 className="text-lg font-semibold text-[#ff3b30] mb-4 flex items-center gap-2">
                  <span className="w-8 h-8 rounded-full bg-[#ff3b30]/10 flex items-center justify-center text-sm">↓</span>
                  Bear Case
                </h3>
                <ul className="space-y-3">
            ...
            <div className="mt-8 pt-8 border-t border-[#d2d2d7]">
              <h3 className="text-lg font-semibold text-[#ff9500] mb-4 flex items-center gap-2">
                <span className="w-8 h-8 rounded-full bg-[#ff9500]/10 flex items-center justify-center text-sm">!</span>
                Risks
              </h3>
              <ul className="space-y-3">
            ...
            {/* Conclusion */}
            <div className="mt-8 pt-8 border-t border-[#d2d2d7]">
              <h3 className="text-lg font-semibold text-[#1d1d1f] mb-4">Conclusion</h3>
              <p className="text-[#6e6e73] leading-relaxed">{research.conclusion}</p>
            </div>
          </div>
        </div>
      </section>
              ...
              {research.agent_analyses.map((agent) => {
                ...
                return (
                  <div key={`analysis-${agent.agent_name}`} className="rounded-3xl bg-white dark:bg-[#2d2d2f] border border-[#d2d2d7] dark:border-[#48484a] p-8 card-hover">
                    ...
                    <p className="text-[#1d1d1f] leading-relaxed mb-6">{agent.summary}</p>
                    ...
                    <div className="mt-6 pt-6 border-t border-[#d2d2d7]">
                      <h4 className="text-sm font-semibold text-[#1d1d1f] mb-2">Conclusion</h4>
                      <p className="text-sm text-[#6e6e73]">{agent.conclusion}</p>
                    </div>
                  </div>
                );
              })}
